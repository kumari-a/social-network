...
function insertpost()
{
	...
	if (isset($_POST['sub'])) 
	{
		...
		if (strlen($content)>2500) {
			...
			echo "<script>windows.open('alumni.php')</script>";
		}
		else
				{
					$insert="INSERT INTO alumni (user_id , post_content , post_date) VALUES ('$user_id' , '$content'  , NOW()) ";
				...
				if ($run) {
			        ...
			        echo "<script>windows.open('alumni.php','_self')</script>";
...
function get_posts(){
	...
	$get_posts="SELECT * FROM alumni ORDER BY 1 DESC LIMIT $start_from,$per_page";
	$run_posts = mysqli_query($conn,$get_posts);

	while ($row_posts = mysqli_fetch_array($run_posts)) {
		$post_id = $row_posts['post_id'];
		$user_id = $row_posts['user_id'];
		$content =substr($row_posts['post_content'], 0,190);
	...
	include("pagination_alumni.php");
}
function single_post(){
	if (isset($_GET['post_id'])) {
		global $conn;
		$get_id=$_GET['post_id'];
		$get_posts="SELECT * FROM alumni WHERE post_id='$get_id'";
		$run_posts=mysqli_query($conn,$get_posts);
		...
		$post_id=$_GET['post_id'];
		$post=$_GET['post_id'];
		$get_user="SELECT * FROM alumni WHERE post_id='$post'";
		...
		if (isset($_POST['comment'])) {
			...
			$insert= "INSERT INTO comments_alumni (post_id,user_id,comment,comment_author,c_date) VALUES ('$post_id','$user_id','$comment','$user_com_name',NOW())";
